"""Experiment config loading and execution."""
...
def run_experiment(config: ExperimentConfig) -> TrainingResult:
  ...
  _write_yaml(run_dir / "config.resolved.yaml", _to_plain(config))

  result = train_on_repo_sources(config.to_train_config())
...
def _apply_overrides(
  ...
  *,
  ...
  return ExperimentConfig(
    ...
    train=config.train
    if steps is None
    else RunTrainConfig(batch_size=config.train.batch_size, steps=steps),
    output=config.output if run_dir is None else OutputConfig(run_dir=run_dir),
  )
